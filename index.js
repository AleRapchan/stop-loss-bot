...
// https://ropsten.etherscan.io/address/0xc0fc958f7108be4060F33a699a92d3ea49b0B5f0
...
// Define the minimum eth to swap
// It can be all the balance, an amount or we could calc a percentage
const ETH_AMOUNT = web3.utils.toWei('0.1', 'Ether') // 1

console.log("Ethereum Amount: ", ETH_AMOUNT)

checkBalances()

// Define Stop Loss Threshold. Related with DAI (USD$)
...
async function sellEth(ethAmount, daiAmount) {
  ...
  const now = moment().unix() // Unix timestamp
  ...
  const SETTINGS = {
    ...
    from: process.env.ACCOUNT, // Account value from .env file
    value: ethAmount // Amount of ETH to Swap
  }

  // Perform Swap on Uniwasp
...
async function monitorPrice() {
  ...
  //console.log();
  process.stdout.write(`\r`, cliSpinners.dots, `Monitoring Ethereum price `)
  ...
  try {
    ...
    if (oldPrice != price){
      console.log('New Eth Price:', price, ' DAI', '@',  new Date().toISOString()) //Date.now()
    oldPrice = price;}

    if(price <= ETH_SELL_PRICE) {
      console.log('Selling Eth...')
      // Checking balance before sale
      await checkBalances()

      // Sell Eth
      await sellEth(ETH_AMOUNT, daiAmount)

      // Checking balances after sale
      await checkBalances()
